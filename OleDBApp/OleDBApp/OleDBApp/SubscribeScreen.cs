...
namespace OleDBApp
{
    public partial class SubscribeScreen : Form
    {
        ...
        {
            string nbBorrow = "select * from Pays order by Nom_Pays";
            OleDbCommand cmd = new OleDbCommand(nbBorrow, dbConnection);
            OleDbDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
            {
                countryCombobox.Items.Add(new Pays(reader.GetInt32(0), reader.GetString(1)));
            }
            reader.Close();
        }
        ...
        {
            string addSubscriber = "insert into Abonné(Nom_Abonné,Prénom_Abonné,Login,Password,Code_Pays) values ('" + userLastName +"','" + userName + "','" + userLogin + "','" + userPass + "'," + countryCode+")";
            OleDbCommand cmd = new OleDbCommand(addSubscriber, dbConnection);
            cmd.ExecuteNonQuery();
        }
        ...
        {
            string logins = "select Login from Abonné";
            OleDbCommand cmd = new OleDbCommand(logins, dbConnection);
            OleDbDataReader reader = cmd.ExecuteReader();

            while (reader.Read())
            {
                if (login.Text.Equals(reader.GetString(0)))
                {
